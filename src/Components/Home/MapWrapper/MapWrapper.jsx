...
function MapWrapper({ eventList }) {
    ...
    const focusMapToLocation = (locationToFocus, center) => {
        ...
        if (center) {
            zoom = 14;
            shiftedLocationToFocus = [locationToFocus[0] + 0.001, locationToFocus[1] - 0.025]
        }

        // Focus to selected event
        else {
            zoom = 18;
            shiftedLocationToFocus = [locationToFocus[0], locationToFocus[1] - 0.0015];
        }
    ...
    useEffect(() => {
        ...
        if (((userLocation[0] !== 0 && userLocation[1] !== 0) && id === undefined) || focusMapToCenter) {
            ...
            </Marker>
            {
                eventList.map((event) => (
                    <Marker
                        ...
                        key={event.id} position={event?.coordinates} icon={
                            new Icon({
